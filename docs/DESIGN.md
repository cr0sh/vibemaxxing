...
Drag-and-drop guidance is contextual, never permanent filler. Hovering or focusing a draggable task, artifact, or shop upgrade, and beginning a native or touch pointer drag, reveals one concise overlay over the whole eligible destination window. A task overlay names the terminal window with an idle agent slot; an artifact overlay names Messenger; an upgrade overlay names the terminal window that can accept it. The overlay is pointer-transparent so native pane handlers and the touch target registry can choose the idle slot. Touch drags use the same source context and reducer actions as native drops, raise eligible windows during the gesture, and keep source capture until drop or cancellation. Empty terminal canvases show only their normal status/cursor until a relevant source is hovered or dragged. Clear hints on source leave (unless a drag is active), drop, drag end, Escape, lost capture, and unmount. Keep the game-only rule above: do not turn these overlays into marketing copy, placeholder sections, or dashboard instructions.
